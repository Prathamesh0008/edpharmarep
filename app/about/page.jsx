...
export default function AboutPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 via-white to-blue-25 relative overflow-hidden">
          ...
          <div className={`space-y-8 transform transition-all duration-1000 ${
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <Link
                href="/ED.pdf"
                download
                className="group relative inline-flex items-center justify-center px-8 py-4 text-sm font-semibold text-white transition-all duration-300 hover:shadow-xl hover:-translate-y-1 overflow-hidden rounded-full"
                style={{ background: `linear-gradient(135deg, ${colors.primary} 0%, ${colors.accent} 100%)` }}
              >
              ...
              <Link
                href="/contact"
                className="inline-flex items-center justify-center px-8 py-4 text-sm font-semibold transition-all duration-300 rounded-full border-2"
                style={{ color: colors.primary, borderColor: `${colors.primary}40`, backgroundColor: "white" }}
                onMouseEnter={(e) => {
          ...
          <div className={`grid grid-cols-2 gap-6 transform transition-all duration-1000 delay-300 ${
            ...
            <div className="col-span-2 relative">
              <div 
                ...
                style={{ background: `linear-gradient(135deg, ${colors.primary} 0%, ${colors.accent} 100%)` }}
              ></div>
              <div className="relative bg-white rounded-3xl p-8 shadow-2xl" style={{ borderColor: `${colors.primary}20`, borderWidth: "1px" }}>
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <span className="text-xs font-semibold tracking-widest" style={{ color: colors.primary }}>
                      {card.tag || "EUROPE TO EUROPE"}
                    </span>
                    <h2 className="text-2xl font-bold text-slate-900">
                      <span style={{ color: colors.primary }}>{card.titleLeft || "ED"}</span>
                      <span style={{ color: colors.accent }}>{card.titleRight || "PHARMA"}</span>
                    </h2>
                  </div>
                </div>
            ...
            {hero.stats && hero.stats.map((stat, index) => (
              <div 
                key={index} 
                className="bg-gradient-to-br from-white to-blue-50 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-shadow duration-300"
      ...
      >
        <div 
          className="bg-gradient-to-br from-white via-blue-50 to-cyan-50 rounded-4xl p-8 lg:p-12 shadow-2xl"
          style={{ borderColor: `${colors.primary}20`, borderWidth: "1px" }}
        >
                ...
                >
                  ...
                  <div className="absolute left-1/2 transform -translate-x-1/2 z-10">
                    <div 
                      ...
                      style={{ borderWidth: "4px", borderColor: colors.primary }}
                    ></div>
                  </div>
      ...
      >
        <div 
          className="bg-gradient-to-br from-blue-50 via-white to-cyan-50 rounded-4xl p-8 lg:p-12 shadow-2xl"
          style={{ borderColor: `${colors.primary}20`, borderWidth: "1px" }}
        >
          <div className="text-center mb-16">
            <div 
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {howWeWork.process && howWeWork.process.map((step, index) => (
              <div key={index} className="relative group">
                <div 
                  className="absolute -inset-1 rounded-3xl blur opacity-20 group-hover:opacity-30 transition-opacity"
                  style={{ background: `linear-gradient(135deg, ${colors.primary} 0%, ${colors.accent} 100%)` }}
                ></div>
                <div 
                  className="relative bg-white rounded-2xl p-6 transition-all duration-300 group-hover:-translate-y-2"
                  style={{ borderColor: `${colors.primary}20`, borderWidth: "1px" }}
                >
                  <div className="flex items-center justify-between mb-6">
                    ...
                    >
                      {index === 0 && <Package className="w-6 h-6" style={{ color: colors.primary }} />}
                      {index === 1 && <ShieldCheck className="w-6 h-6" style={{ color: colors.primary }} />}
                      {index === 2 && <Truck className="w-6 h-6" style={{ color: colors.primary }} />}
                      {index === 3 && <Users className="w-6 h-6" style={{ color: colors.primary }} />}
                    </div>
                  </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {focus.items && focus.items.map((item, index) => (
            ...
            >
              ...
              {/* Card Content */}
              <div 
                className="relative bg-gradient-to-b from-white to-blue-50 rounded-2xl p-8 transition-all duration-500 group-hover:-translate-y-2"
                style={{ borderColor: `${colors.primary}20`, borderWidth: "1px" }}
              >
                {/* Icon Background */}
                <div className="mb-6">
                  <div 
                    ...
                    style={{ background: `${colors.primary}10` }}
                  >
                    {index === 0 && <Package className="w-8 h-8" style={{ color: colors.primary }} />}
                    {index === 1 && <Building className="w-8 h-8" style={{ color: colors.primary }} />}
                    {index === 2 && <Globe className="w-8 h-8" style={{ color: colors.primary }} />}
                  </div>
                </div>
