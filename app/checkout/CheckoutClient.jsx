...
export default function CheckoutClient() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        // Check if cart is empty and we're NOT redirecting
        if (cartItems.length === 0 && !isRedirecting) {
          setIsLoading(false);
          return;
        }

        // If cart is empty but we're redirecting, don't show empty state
        if (cartItems.length === 0 && isRedirecting) {
          setIsLoading(false);
          return;
        }
        ...
        if (addresses.length > 0) {
          ...
          // If no saved addresses, pre-fill with user data from registration
          setForm(prevForm => ({
            ...prevForm,
            fullName: user.username || prevForm.fullName,
            email: user.email || prevForm.email,
            phone: user.mobile || prevForm.phone,
            address: user.street || prevForm.address,
            city: user.city || prevForm.city,
            pincode: user.pincode || prevForm.pincode,
          }));
        }
      } catch (error) {
    ...
    checkAuth();
  }, [router, cartItems.length, isRedirecting]);
  ...
  const handleLoginSuccess = (user) => {
    ...
    // Auto-fill form with user data from registration
    setForm(prevForm => ({
      ...prevForm,
      fullName: user.username || prevForm.fullName,
      email: user.email || prevForm.email,
  ...
  const placeOrder = async () => {
    try {
      ...
      // CRITICAL: Set redirecting state to true BEFORE clearing cart
      // This prevents the empty cart check from showing
  ...
  // Show empty cart state only if not redirecting
  if (cartItems.length === 0 && !isRedirecting) {
    return (
      <>
        <Navbar />
  ...
  const isDisabled = !currentUser || validateForm(form, cartItems, payment) !== null || isLoading || isRedirecting;
              ...
              >
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    disabled={isDisabled}
                    className={[
                      "mt-5 w-full py-3.5 rounded-xl text-sm sm:text-base font-semibold cursor-pointer",
                      isDisabled
...
// UI Components (keep all existing UI components unchanged)
