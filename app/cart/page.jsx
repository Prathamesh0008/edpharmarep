...
export default function CartPage() {
  ...
  const handleLoginSuccess = (user) => {
    ...
    // If checkout was pending, redirect to checkout
    if (pendingCheckout) {
          ...
          {hasItems && (
            <div className="flex flex-col sm:items-end">
              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">
                ...
                {totals.totalBulkUnits} {totals.totalBulkUnits !== 1 ? 'batches' : 'batch'}
              </span>
              <span className="text-xs text-gray-500 mt-1 text-right">
                {totals.totalQty.toLocaleString()} total units
              </span>
            </div>
          )}
        </div>
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                      {/* IMAGE */}
                      <div className="relative w-24 h-24 sm:w-24 sm:h-24 md:w-28 md:h-28 bg-slate-50 rounded-lg sm:rounded-xl border border-slate-100 overflow-hidden shrink-0 mx-auto sm:mx-0">
                        <Image
                      ...
                      <div className="flex-1 flex flex-col min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-2 mt-1">
                              <p className="inline-flex items-center gap-1 text-xs sm:text-sm text-gray-500">
                                <Euro size={12} className="sm:size-[14px]" />
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-4 pt-3 border-t border-slate-100">
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center sm:justify-start gap-1 text-xs sm:text-sm text-red-500 hover:text-red-600 px-3 py-1.5 rounded-lg hover:bg-red-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {isBeingRemoved ? (
                              <>
                ...
                <div className="relative">
                  <div className="flex items-center justify-between gap-2 mb-3 sm:mb-4">
                    ...
                    <span className="text-[10px] sm:text-[11px] font-medium text-gray-500">
                      {totals.totalDistinct} {totals.totalDistinct > 1 ? 'items' : 'item'}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm">
                    ...
                    <div className="p-2 sm:p-3 bg-blue-50/50 rounded-lg border border-blue-100 mb-2 sm:mb-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-blue-600">
                          {INITIAL_BULK_QUANTITY} units/batch
                        </span>
                      </div>
                    </div>
                    
                    <div className="flex justify-between py-1 sm:py-1.5">
                      <span className="text-xs sm:text-sm text-gray-600">Total Units</span>
                    ...
                    <div className="flex justify-between py-1 sm:py-1.5">
                      <span className="text-xs sm:text-sm text-gray-600">Total Batches</span>
                      <span className="text-xs sm:text-sm font-medium text-slate-900">
                        {totals.totalBulkUnits}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between py-1 sm:py-1.5">
                      ...
                      <span className="text-xs sm:text-sm font-bold text-[#0A4C89]">
                        €{totals.totalPrice.toLocaleString()}
                      </span>
                    </div>
                  </div>
                  ...
                  {!isLoading && !isUserLoggedIn && (
                    <div className="my-3 sm:my-4 p-2 sm:p-3 bg-yellow-50 border border-yellow-100 rounded-lg">
                      ...
                      <button
                        ...
                        className="text-xs font-medium text-[#0A4C89] hover:underline cursor-pointer"
                      >
                        Click here to login
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className={[
                      "mt-3 sm:mt-4 w-full py-2.5 sm:py-3.5 rounded-lg sm:rounded-xl text-sm sm:text-base font-semibold cursor-pointer",
