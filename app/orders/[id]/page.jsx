...
const downloadInvoice = async () => {
  try {
    ...
    // Company Header
    doc.setFontSize(24);
    doc.setTextColor(10, 76, 137); // #0A4C89
    doc.text('ED Pharma', 14, 22);
    
    doc.setFontSize(12);
    doc.setTextColor(100, 116, 139);
    doc.text('Your Trusted Pharmacy Partner', 14, 30);
    ...
    const invoiceId = `Invoice #: INV-${order?.orderId || ''}`;
    const orderId_text = `Order ID: ${order?.orderId || ''}`;
    const dateText = `Date: ${formatDate(order?.createdAt) || ''}`;
    
    doc.text(invoiceId, 14, 55);
    doc.text(orderId_text, 14, 61);
    doc.text(dateText, 14, 67);
    ...
    doc.setFontSize(10);
    doc.setTextColor(75, 85, 99);
    doc.text(order?.address?.fullName || 'Customer', 14, 90);
    ...
    autoTable(doc, {
      ...
      body: tableRows,
      theme: 'striped',
      headStyles: { fillColor: [10, 76, 137], textColor: 255, fontStyle: 'bold' },
    ...
    // Order Summary
    const subtotal = order?.totals?.totalPrice || 0;
    ...
    doc.text(`€${subtotal.toFixed(2)}`, summaryX + 50, summaryY, { align: 'right' });
    
    summaryY += 7;
    doc.text('Shipping:', summaryX, summaryY);
    doc.text(order?.shippingCost ? `€${shipping.toFixed(2)}` : 'FREE', summaryX + 50, summaryY, { align: 'right' });
    
    summaryY += 7;
    doc.text('Tax:', summaryX, summaryY);
    doc.text(`€${tax.toFixed(2)}`, summaryX + 50, summaryY, { align: 'right' });
    
    summaryY += 10;
    doc.setFontSize(12);
    doc.setTextColor(10, 76, 137);
    doc.setFont(undefined, 'bold');
    doc.text('Total:', summaryX, summaryY);
    doc.text(`€${total.toFixed(2)}`, summaryX + 50, summaryY, { align: 'right' });
    
    // Payment Information
    doc.setFontSize(11);
    doc.setTextColor(31, 41, 55);
    doc.setFont(undefined, 'bold');
    doc.text('Payment Information', 14, summaryY + 20);
    ...
    const paymentStatus = order?.status?.toLowerCase().includes('cancel') || order?.status?.toLowerCase().includes('reject') 
      ? 'Cancelled' 
      : order?.paymentMethod === 'cod' 
        ? 'Pending (Pay on Delivery)' 
        : 'Paid';
    
    doc.text(`Payment Method: ${paymentMethod}`, 14, summaryY + 28);
    doc.text(`Payment Status: ${paymentStatus}`, 14, summaryY + 35);
    doc.text(`Order Status: ${order?.status || 'N/A'}`, 14, summaryY + 42);
    ...
    doc.text('ED Pharma - Your Trusted Pharmacy Partner', 14, pageHeight - 15);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30 pt-6 pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Customer Support - Now with 2 cards only, aligned properly */}
        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
          {/* Track Order / Reorder Card */}
          ...
          <div className="bg-gradient-to-br from-violet-600 to-violet-700 rounded-2xl p-6 text-white">
            ...
            <p className="text-sm font-medium mb-2">sales@edpharma.co</p>
